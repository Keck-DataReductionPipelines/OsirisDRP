...
; set filename
if keyword_set(pname) then printer=pname else printer=''
if keyword_set(wid_lead) then wid=wid_lead else wid=0L 
if keyword_set(osys) then os=osys else os="UNIX" 
...
Function cprint::cmps_form, xoffset, yoffset, Cancel=cancelButton, Help=help, $
     ...
     xpagesize=xpagesize, ypagesize=ypagesize, pagebox=pagebox, Printdes=printdes
...
;   bitslabel = Widget_Label(opttlb, Value='   Color Bits:')
   ...
   fbase = Widget_Base(filenamebase, /row, /Exclusive)
   ...
   pbase = widget_base(filenamebase, /row, /Exclusive))
   pfilenamew = Widget_Text(pbase, /Editable, Scr_XSize=300,  $
      Value='', event_pro='cprint_cmps_form_null_events')
   pfilenameb = widget_button(pbase, value='Choose...', $
                             event_pro='cprint_cmps_form_select_file_event')
   
   ptextlabel=widget_label(pbase, Value='Printer Name: ')
   ...
   if n_elements(buttons) GT 0 then begin
       for i = 0, n_elements(buttons)-1 do begin
           but = widget_button(actionbuttonbase, value=buttons(i), $
                               uvalue='ACCEPT')
                  print = Widget_Button(actionbuttonbase, Value='Print File', $
                              UValue='PRINT')
              endfor
   endif else begin
...
if result[0] eq '' then begin
    ...
    command=strcompress('lpr -P'+ self.printer_name+' '+print_fn)
    print, command
...
print, 'Windows printing not supported'
...
pro CPrint::SetPrinter_name, printer_name

self.Printer_name=data_ptr

end

function CPrint::GetPrinter_name
...
pro CPrint::SetOperating_system, operating_system

self.operating_system=operating_system

end

function CPrint::GetOperating_system
...
function CPrint::GetBits_per_pixel
