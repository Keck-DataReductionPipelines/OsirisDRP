pro ql_aper,image,xc,yc,mags,errap,sky,skyerr,phpadu,apr,skyrad,badpix, $
       SETSKYVAL = setskyval,PRINT = print, SILENT = silent, FLUX=flux, $
       EXACT = exact, NUMAPER_PIX=numaper_pix, BASE_ID=base_id
;+
; NAME:
;      QL_APER
; PURPOSE:
...
;               will be simply ignored for the sky computation.
;
...
;       Default now no bad pixels, check for bad pixels prior to sky computation
...
; set the default aperture pixels to -1 
numaper_pix=-1           
...
;       below BADLO threshold.  SKYBUF will be 1-d array of sky pixels
 ...
 if chk_badpix then skypix = skypix and ( rotbuf GT badpix[0] ) and $
        (rotbuf LT badpix[1] )
 ...
 if ( nsky LT minsky ) then begin                       ;Sufficient sky pixels?
  ...
  ; make sure the skybuf has more than 20 elements, otherwise mmm.pro will crash
  ; the 20 sky elements are a minimum sky by mmm.pro, line 69
  if (n_elements(skybuf) le 20) then begin
      return
  endif else begin
...
for k = 0,Naper-1 do begin      ;Find pixels within each aperture
    ...
    if ( edge[i] LT apr[k] ) then begin ;Does aperture extend outside the image?
        apmag[k] = -1.0E36
        numaper_pix=-1           
    endif else begin
        if keyword_set(EXACT) then begin
        ...
        numaper_pix=n_elements(thisapd)
