...
cprint_obj=obj_new('cprint', wid_leader=base)
...
self.cprint_obj=ptr_new(cprint_obj)
...
cprint_ptr=self.cprint_obj
cprint_obj=*cprint_ptr
...
cancelled=cprint_obj->DeviceSetup()
self->DrawImage, /PS

if not cancelled then begin
cprint_obj->DeviceCleanup
endif
...
;        print, 'stop'
;        print, printdes, print_fn

;        if printdes then begin
          ;check printer existence
;          lpq_command=strcompress('lpq -P'+' q6171')
;          spawn, lpq_command, result

;          if result[0] eq '' then begin
;              answer=dialog_message(['Printer queue: ', ' q6171', $
;                                     'not found.'], dialog_parent=base_id, /error)
;          endif else begin
              ; print it!
;              print, 'Printing on ', ' q6171'
;              command=strcompress('lpr -P'+' q6171'+' '+print_fn)
;              print, command
;              spawn, command, result
              ; if error (actually, i don't think result is ever not '', but in case...)
;              if result[0] ne '' then begin
;                  answer=dialog_message(result, dialog_parent=base_id, /error)
;              endif
;          endelse
;endif
...
if check_result eq 'Yes' then begin
    ...
    ; necessary
    disp_min = self->GetDispMin()
    disp_max = self->GetDispMax()

    print, disp_min, disp_max

    disp_im = imdata > disp_min
    disp_im = disp_im < disp_max
    if base_uval.log_scale eq 'yes' then begin
        disp_im = alog(disp_im)
    endif

    ; make the aspect ratio of the printed image correspond to the data
    ratio = float(im_s[1]) / float(im_s[0])
    xs=5
    ys = xs * ratio
    ...
    z0=self.ZMin
    z1=self.ZMax
    collapsed_im=total(reform(disp_im[*, *, z0:z1], im_s[0], im_s[1], z1-z0+1), 3)/(z1-z0+1)

    ; plot the pixel values along the edge then print the image to the ps file
    plot, [0,im_s(0)], [0,im_s(1)], pos=[0,0,im_s(0),im_s(1)], xticks=3, yticks=3, $
      xticklen=-0.01, yticklen=-0.01, $
      /nodata, /noerase, title=title, xtitle='pixels', ytitle='pixels', $
      xstyle=1, ystyle=1
    tvscl, collapsed_im, 0,0 , xs=xs, ys=ys, /inches 
...
pro cimwin__define
	...
	p_PlotWin=ptr_new(/allocate_heap)
        ; make an instance of the print object
        cprint_obj=ptr_new(/allocate_heap)
	...
	struct={CImWin, $
		...
		p_PlotWin:p_PlotWin, $		; pointer to Plot Window object
                cprint_obj:cprint_obj, $        ; pointer to the print object
