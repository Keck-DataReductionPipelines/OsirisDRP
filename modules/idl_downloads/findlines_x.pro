...
function findlines_x, p_WFrame_, p_WIntFrame_, p_WIntAuxFrame_, d_MinDiff, $
...
;                                        MEASURE_ERRORS = reform(cd_LineFitCoeffErr(i1,i2,1,vi_FitLine)), $
   ...
   if ( keyword_set ( FitCoeff ) ) then begin
      ...
      cd_CoeffFitCoeff = dblarr(n_Dims(3), n_FitOrder+1, i_CoeffFitOrder+1 ) ; coefficients of the fitted fit coefficients
      ...
      ; loop over the x-slices
      for i=0, n_Dims(3)-1 do begin
         ...
         for j=0, n_FitOrder do begin
            ...
            vd_X     = dindgen(n_Dims(2))                                         ; the running index of the x-slice
            ...
            if ( n_Valid gt i_CoeffFitOrder ) then begin 
               ...
               vd_X = vd_X(vi_Valid)
               vd_Y = cd_DispFitCoeff(vi_Valid,i,j)
               vd_W = cd_DispFitCoeffErr(vi_Valid,i,j)
               ...
               if ( n_Clip ge i_CoeffFitOrder ) then begin
                  ...
                  if ( NOT bool_is_vector ( vd_Coeff ) ) then begin
                     ...
                     for k=0, n_Valid-1 do $
                        cd_CoeffCoeff(vi_Valid(k), i, j ) = poly(vi_Valid(k),vd_Coeff) 

                     mb_CoeffStat(vi_Valid,i)  = ERROR_COEFF_OK
