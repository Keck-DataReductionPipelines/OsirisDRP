...
;  use of /bin/sh in the line spawn, ucmprs+filename, unit=unit,/sh
        ...
        if gzip then begin
                ...
                if (!version.os_family EQ 'unix') then begin
                        ...
                        MESSAGE, 'Only Unix IDL supports piped spawns. ' + $
				 'File must be uncompressed manually'                     
                endelse
...
;  Read the next header, and get the number of bytes taken up by the data.
       ...
       if EOF(unit) then begin 
		if not unitsupplied then free_lun,unit
            	message, 'EOF encountered attempting to read extension ' + strtrim(ext,2)
       endif
      ...
      if (ext EQ 0 ) and (keyword_set(pointlun) EQ 0) then $
		...
		if not unitsupplied then free_lun, unit
             	message,'Header does not contain required SIMPLE keyword'
      	   endif
...
;  Move to the next extension header in the file.
 ...
 case BITPIX of 
		...
		if not unitsupplied then free_lun,unit
                message,'Illegal value of BITPIX (= ' +  $
 ...
 if Naxis GT 0 then begin 
  ...
  if nbytes EQ 0 then begin
	...
	if not SILENT then message, $
                "FITS header has NAXIS or NAXISi = 0,  no data array read"
 endif
...
; to read?
   ...
   if (exten_no GT 0) and ((startrow NE 0) or (numrow NE nrow)) then begin
	   ...
	   if not unitsupplied then free_lun,unit
          ...
          ' but only ' + strtrim(nax[1],2) + ' rows in extension'
        endif 
