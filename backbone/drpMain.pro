...
; 2005/09/21 JLW: use single better named envvar 
...
;	CONFIG_FILENAME_FILE = BACKBONE_SUPPORT_DIR + CONFIG_FILENAME_FILE
        CONFIG_FILENAME_FILE = GETENV('OSIRIS_DRP_BACKBONE_CONFIG_FILE')
	CONFIG_FILENAME = ''
	CONFIG_FILEUNIT = 3

	CALLABLE_INSTANTIATION = 0
	DEFSYSV, '!CALLED_FROM_C', EXISTS=CALLABLE_INSTANTIATION
; 2005/09/21 JLW: changed to better envvar name
;	externalCodeDir = GETENV('EXTERNAL_CODE_DIR')
	externalCodeDir = GETENV('OSIRIS_DRP_EXTERNAL_LIB_DIR')
...
; 2005/09/21 JLW: changed name of library (see drs/modules/source)
;		EXTERNAL_CODE_FILENAME = externalCodeDir + 'osiris_ext_null.so'
		EXTERNAL_CODE_FILENAME = externalCodeDir + 'libosiris_drp_ext_null.so.0.0'
	ENDELSE
